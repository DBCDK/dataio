...
import dk.dbc.dataio.jobrerunrules.service.JobRerunScheme;
...
public class JobRerunSchemeParserTest {
    ...
    public void parse_previewJob_hasActionCopy() throws FlowStoreServiceConnectorException {
        ...
        final JobRerunSchemeParser jobRerunSchemeParser = getJobRerunSchemeParser();
    ...
    public void parse_toTickleJob_hasActionRerunAndRerunAllIsTypeTickle() throws FlowStoreServiceConnectorException {
        ...
        final JobRerunSchemeParser jobRerunSchemeParser = getJobRerunSchemeParser();
        when(jobRerunSchemeParser.flowStoreServiceConnectorBean.getConnector()
            .getSink(anyLong())).thenReturn(new Sink(1, 1,
            new SinkContentBuilder().setSinkType(SinkContent.SinkType.TICKLE)
            .build()));
    ...
    public void parse_toTickleJob_hasActionRerunIsTypeTickle() throws FlowStoreServiceConnectorException {
        ...
        when(jobRerunSchemeParser.flowStoreServiceConnectorBean.getConnector()
            .getSink(anyLong())).thenReturn(new Sink(1, 1,
            new SinkContentBuilder().setResource(JobRerunScheme.TICKLE_TOTAL)
            .setSinkType(SinkContent.SinkType.TICKLE).build()));
    ...
    public void parse_fromRawRepoJob_hasActionRerunAndRerunAllIsTypeRawRepo() throws FlowStoreServiceConnectorException {
        ...
        when(jobRerunSchemeParser.flowStoreServiceConnectorBean.getConnector()
            .getSink(anyLong())).thenReturn(new Sink(1, 1,
            new SinkContentBuilder().setSinkType(SinkContent.SinkType.HIVE)
            .build()));
    ...
    private static JobRerunSchemeParser getJobRerunSchemeParser() {
        final FlowStoreServiceConnectorBean flowStoreServiceConnectorBean =
            mock(FlowStoreServiceConnectorBean.class);
        final FlowStoreServiceConnector flowStoreServiceConnector = mock(
            FlowStoreServiceConnector.class);
        when(flowStoreServiceConnectorBean.getConnector()).thenReturn(
            flowStoreServiceConnector);
        final JobRerunSchemeParser parser = new JobRerunSchemeParser();
        parser.flowStoreServiceConnectorBean = flowStoreServiceConnectorBean;
        return parser;
    }
}

...
public class FlowCacheTest {
    ...
    static {
        try {
            FLOW_COMPONENT = new FlowComponentBuilder().setContent(ChunkProcessorTest.getFlowComponentContent(new ChunkProcessorTest.ScriptWrapper("test", ChunkProcessorTest.getJavaScript(ChunkProcessorTest.getJavaScriptReturnUpperCaseFunction())))).build();
            FLOW = new FlowBuilder().setContent(new FlowContentBuilder().setComponents(Collections.singletonList(FLOW_COMPONENT)).build()).build();
        } catch (Exception e) {
    ...
    public void put_flowArgContainsNoComponents_throws() throws Throwable {
        Flow emptyFlow = new FlowBuilder().setContent(new FlowContentBuilder().setComponents(Collections.emptyList()).build()).build();
        try {
    ...
    public void put_flowArgHasNoNextComponents_addsAndReturnsCacheEntry() throws Throwable {
        FlowCache.FlowCacheEntry entry = cache.put(KEY_FOUND, FLOW);
    ...
    public void put_flowArgHasNextComponents_addsAndReturnsCacheEntry() throws Throwable {
        ChunkProcessorTest.ScriptWrapper ucWrapper = new ChunkProcessorTest.ScriptWrapper("test", ChunkProcessorTest.getJavaScript(ChunkProcessorTest.getJavaScriptReturnUpperCaseFunction()));
        FlowComponent flowComponent = new FlowComponentBuilder().setContent(ChunkProcessorTest.getFlowComponentContent(ucWrapper)).setNext(ChunkProcessorTest.getFlowComponentContent(ucWrapper)).build();

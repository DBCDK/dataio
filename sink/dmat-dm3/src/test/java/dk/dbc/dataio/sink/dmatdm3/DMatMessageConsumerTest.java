...
public class DMatMessageConsumerTest {
    ...
    private byte[] readLocalFile(String name) throws IOException {
        Path path = Paths.get(Objects.requireNonNull(DMatMessageConsumerTest.class.getResource("/__files/" + name)).getPath());
    ...
    public void testRecordData() throws IOException, JSONBException {
        ...
        RecordData dmatRecord = RecordData.fromRaw(new String(addiRecord.getContentData()));

        assertThat("datestamp", dmatRecord.getDatestamp(), Is.is("20150706T12:09:19Z"));
        assertThat("recordReference", dmatRecord.getRecordReference(), Is.is("33707b62-99d2-47a0-ad51-a7a35e53493c"));
        assertThat("active", dmatRecord.getActive(), Is.is(true));
        assertThat("isbn13", dmatRecord.getIsbn13(), Is.is("9788772196244"));
        assertThat("productForm", dmatRecord.getProductForm(), Is.is("DIGITAL_DOWNLOAD"));
        assertThat("contentType", dmatRecord.getContentType(), Is.is("TEXT_EYE_READABLE"));
        assertThat("title.text", dmatRecord.getTitle().getText(), Is.is("Leadership pipeline"));
        assertThat("publisherName", dmatRecord.getPublisherName(), Is.is("Gyldendal"));
        assertThat("publishingDate", dmatRecord.getPublishingDate(), Is.is("20150202"));
        assertThat("lendingTypes", dmatRecord.getLendingTypes(), Is.is(notNullValue()));
        assertThat("lendingTypes", dmatRecord.getLendingTypes().size(), Is.is(1));
        assertThat("lendingTypes contains ERL", dmatRecord.getLendingTypes().get(0), Is.is(EReolCode.ERE));
    }
}

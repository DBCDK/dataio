...
public class DependencyTrackingIT extends AbstractJobStoreIT {
    ...
    public void querySinkIdStatusCount() throws Exception {
        ...
        assertThat("result[0]", result.get(0), is(new SinkIdStatusCountResult(1, ChunkSchedulingStatus.READY_FOR_PROCESSING, 5)));
        assertThat("result[1]", result.get(1), is(new SinkIdStatusCountResult(1, ChunkSchedulingStatus.QUEUED_FOR_PROCESSING, 4)));
        assertThat("result[2]", result.get(2), is(new SinkIdStatusCountResult(1, ChunkSchedulingStatus.BLOCKED, 2)));
        assertThat("result[3]", result.get(3), is(new SinkIdStatusCountResult(1, ChunkSchedulingStatus.READY_FOR_DELIVERY, 3)));
        assertThat("result[4]", result.get(4), is(new SinkIdStatusCountResult(1, ChunkSchedulingStatus.QUEUED_FOR_DELIVERY, 1)));

        assertThat("result[5]", result.get(5), is(new SinkIdStatusCountResult(1551, ChunkSchedulingStatus.READY_FOR_PROCESSING, 1)));
        assertThat("result[6]", result.get(6), is(new SinkIdStatusCountResult(1551, ChunkSchedulingStatus.QUEUED_FOR_PROCESSING, 2)));
        assertThat("result[7]", result.get(7), is(new SinkIdStatusCountResult(1551, ChunkSchedulingStatus.BLOCKED, 3)));
        assertThat("result[8]", result.get(8), is(new SinkIdStatusCountResult(1551, ChunkSchedulingStatus.READY_FOR_DELIVERY, 4)));
        assertThat("result[9]", result.get(9), is(new SinkIdStatusCountResult(1551, ChunkSchedulingStatus.QUEUED_FOR_DELIVERY, 5)));
    }

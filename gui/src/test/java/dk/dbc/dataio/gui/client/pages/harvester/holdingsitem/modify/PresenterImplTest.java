...
public class PresenterImplTest extends PresenterImplTestBase {
    ...
    public void nameChanged_null_noAction() {
        // Test preparation
        presenter.start(mockedContainerWidget, mockedEventBus);
        presenter.setHoldingsItemHarvesterConfig(null);
    ...
    public void nameChanged_validName_nameSet() {
        // Test preparation
        presenter.start(mockedContainerWidget, mockedEventBus);
        presenter.setHoldingsItemHarvesterConfig(mockedConfig);
    ...
    public void rrHarvesterChanged_null_noAction() {
        ...
        // Test
        presenter.rrHarvestersChanged(Arrays.asList("rrHarvester"));
    ...
    public void rrHarvesterChanged_validRrHarvester_rrHarvesterSet() {
        ...
        // Test
        presenter.rrHarvestersChanged(Arrays.asList("123"));
        ...
        list.add(123L);
        verify(mockedContent).withRrHarvesters(list);
        commonPostVerification();
    }
    ...
    public void saveButtonPressed_configNonEmpty_errorMessageDisplayed() {
        ...
        ArrayList<Long> list = new ArrayList<>();
        list.add(234L);
        when(mockedContent.getRrHarvesters()).thenReturn(list);
    ...
    private void verifyInitializeViewFields(int statusCount) {
        ...
        verifyRrHarvesters(mockedRrHarvesters, "", false);
        verify(mockedRrHarvesters).setEnabled(false);
        verify(mockedEnabled).setValue(false);
        verify(mockedEnabled).setEnabled(false);
        verify(mockedRrHarvestersListBox).addItem("RR1", "11");
        verify(mockedRrHarvestersListBox).addItem("RR2", "22");
        verify(mockedRrHarvestersListBox).addItem("RR3", "33");
        verify(mockedStatus, times(statusCount)).setText("");
    }

    private void verifyRrHarvesters(PromptedMultiList list, String value, boolean enabled) {
        verify(list).clear();
    ...
    private void commonPostVerification() {
        ...
        verifyNoMoreInteractions(mockedRrHarvestersListBox);

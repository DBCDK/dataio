...
public class HarvestersTable extends CellTable {
    ...
    Presenter presenter;
    ListDataProvider<PromatHarvesterConfig> dataProvider;
    SingleSelectionModel<PromatHarvesterConfig> selectionModel = new SingleSelectionModel<>();
    ...
    public void setHarvesters(Presenter presenter, List<PromatHarvesterConfig> harvesters) {
        ...
        if (!harvesters.isEmpty()) {
            for (PromatHarvesterConfig config : harvesters) {
    ...
    private Column constructNameColumn() {
        return new TextColumn<PromatHarvesterConfig>() {
            @Override
            public String getValue(PromatHarvesterConfig config) {
    ...
    private Column constructActionColumn() {
        Column column = new Column<PromatHarvesterConfig, String>(new ButtonCell()) {
            @Override
            public String getValue(PromatHarvesterConfig config) {
                // The value to display in the button.
                return texts.button_EditPromatHarvesterButton();
            }
        };
        column.setFieldUpdater(
                (FieldUpdater<PromatHarvesterConfig, String>)
    ...
    private void editConfig(PromatHarvesterConfig config) {
        if (config != null) {
            presenter.editPromatHarvester(String.valueOf(config.getId()));
        }
    }
}

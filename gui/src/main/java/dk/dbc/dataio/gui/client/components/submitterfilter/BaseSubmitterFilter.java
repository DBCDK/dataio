...
public abstract class BaseSubmitterFilter extends Composite implements HasChangeHandlers, Focusable {
    ...
    SubmitterFilter parentFilter = null;
    SubmitterFilterPanel filterPanel = null;
     ...
     */
    Scheduler.ScheduledCommand getAddCommand(final SubmitterFilter parentJobFilter) {
    ...
    public void instantiateFilter(boolean notifyPlace) {
        if (filterPanel == null) {
            filterPanel = new SubmitterFilterPanel(getName(), resources, initialInvertFilterValue);
            clickHandlerRegistration = filterPanel.addSubmitterFilterPanelHandler(event -> handleFilterPanelEvent(event.getFilterPanelButton()));

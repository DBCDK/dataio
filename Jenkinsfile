    ...
    stages {
        ...
        stage("deploy this branch to staging?") {
            ...
            steps {
                ...
                sh """
                mvn deploy -B -Dmaven.test.skip=true -Pdocker-push -Dtag="${env.BRANCH_NAME}-${env.BUILD_NUMBER}" -am -pl "commons/utils/flow-store-service-connector, commons/utils/tickle-harvester-service-connector, gatekeeper, job-processor2, sink/dummy, dlq-errorhandler, sink/dmat"

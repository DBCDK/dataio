...
public class PresenterEditImpl<Place extends EditPlace> extends PresenterImpl {
    ...
    public void initializeModel() {
        commonInjector.getFlowStoreProxyAsync().getHoldingsItemHarvesterConfig(id, new GetHoldingsItemHarvesterConfigAsyncCallback());
    }
    ...
    @Override
    void saveModel() {
        commonInjector.getFlowStoreProxyAsync().updateHarvesterConfig(config, new UpdateHoldingsItemHarvesterConfigAsyncCallback());
    }
    ...
    public void deleteButtonPressed() {
        commonInjector.getFlowStoreProxyAsync().deleteHarvesterConfig(config.getId(), config.getVersion(), new AsyncCallback<Void>() {
            ...
            public void onFailure(Throwable e) {
                String msg = "HoldingsItemHarvesterConfig.id: " + config.getId();
                getView().setErrorText(ProxyErrorTranslator.toClientErrorFromFlowStoreProxy(e, commonInjector.getProxyErrorTexts(), msg));
            }
            @Override
            public void onSuccess(Void aVoid) {
                getView().status.setText(getTexts().status_HoldingsItemHarvesterSuccessfullyDeleted());
                setHoldingsItemHarvesterConfig(null);
                History.back();
            }
        });
    }
    ...
    class UpdateHoldingsItemHarvesterConfigAsyncCallback implements AsyncCallback<HarvesterConfig> {
        @Override
        public void onFailure(Throwable e) {
            String msg = "HoldingsItemHarvesterConfig.id: " + id;
            getView().setErrorText(ProxyErrorTranslator.toClientErrorFromFlowStoreProxy(e, commonInjector.getProxyErrorTexts(), msg));

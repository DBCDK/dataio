...
public class SortableList extends Composite implements HasValue<Map<String, String>> {
    interface SortableListUiBinder extends UiBinder<HTMLPanel, SortableList> {}
    private static SortableListUiBinder ourUiBinder = GWT.create(SortableListUiBinder.class);

    private ValueChangeHandler<Map<String, String>> valueChangeHandler = null;
    private SortableListModel model = new SortableListModel();
    ...
    private class SortableListModel {
        ...
        Map<String, String> get() {
            Map<String, String> result = new HashMap<String, String>(modelWidgets.size());
            for (SortableWidget widget: modelWidgets) {
         ...
         */
        private void setOneSelected(GQuery selectWidget) {
    ...
    /**
     * Adds one item to the bottom of the list
     * It triggers a ValueChangedEvent if the supplied fireEvent boolean parameter is true
     * @param text The text as it is displayed in the list
     * @param key The key for the list item
     * @param fireEvent A boolean to determine, if an event is being fired upon change
     */
    public void add(String text, String key, boolean fireEvent) {
        model.add(text, key);
        fireEvent(fireEvent);
    }
    ...
    /**
     * Fire a ValueChangeEvent, if the supplied parameter is true
     * @param fireEvent If true: fire the ValueChangeEvent
     */
    private void fireEvent(boolean fireEvent) {
        if (fireEvent) {
            fireEvent();
        }
    }
    ...
    /**
     * Fires a ValueChangeEvent, causing the ValueChangeHandler to be activated if set
     */
